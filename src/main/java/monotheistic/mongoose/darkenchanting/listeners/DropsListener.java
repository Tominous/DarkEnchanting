...
import monotheistic.mongoose.darkenchanting.utils.Range;
...
public class DropsListener extends Component {
    ...
    private Map<Tier, Range> killValues;

    public DropsListener(Main main) {
        onEnable(() -> killValues = parseKills(main));
        onDisable(()->killValues=null);
    ...
    private void addDrops(Range range, EntityDeathEvent event) {
        if (range == null)
            return;
    ...
    public Map<Tier, Range> parseKills(JavaPlugin plugin) {
        Map<Tier, Range> map = new HashMap<>();
        final FileConfiguration configuration = new Configuration(plugin, "options.yml").configuration();
        ...
        if (playersEnabled) {
            ...
            map.put(Tier.PLAYER, new Range(configuration.getInt("player-amount.chance"), MiscUtils.parseInt(playersStringArr[0]).orElse(10), MiscUtils.parseInt(playersStringArr[1]).orElse(15)));
        }
        if (hostilesEnabled) {
            final String[] playersStringArr = configuration.getString("hostile-amount.amount").split("-");
            map.put(Tier.HOSTILE, new Range(configuration.getInt("hostile-amount.chance"), MiscUtils.parseInt(playersStringArr[0]).orElse(3), MiscUtils.parseInt(playersStringArr[1]).orElse(5)));
        }
        if (friendliesEnabled) {
            final String[] playersStringArr = configuration.getString("nonhostile-amount.amount").split("-");
            map.put(Tier.NON_HOSTILE, new Range(configuration.getInt("nonhostile-amount.chance"), MiscUtils.parseInt(playersStringArr[0]).orElse(1), MiscUtils.parseInt(playersStringArr[1]).orElse(3)));
        }
        return map;
    }
